...
このAI駆動開発テンプレートはコンテキストエンジニアリングを取り入れています。
コンテキストエンジニアリングは、あらかじめ実装する内容をコンテキストファイルに保存しておき、AIはこのコンテキストファイルを元に実装していく方法です。


### コンテキストエンジニアリングのメリット

コンテキストエンジニアリングを実践することで以下のようなメリットがあります。

1. **実装方針が人間にもわかりやすくなる**
   従来の方針（例えば「ログイン機能を実装して」）で指示を与えた場合、ログイン機能の実装方針やファイル構成などはAIにお任せとなってしまいます。
   コンテキストファイルをあらかじめ記載しておくことで、どのファイルに何を実装するのか事細かに指示を与えることが可能になります。
2. **コンテキストの節約**
   AIは計画段階でファイル探索やコマンド実装の際に余計な情報まで取得することがあり、結果としてコンテキストを圧迫してしまうことになります。（例えば違ったファイルを読み込んでしまい、それに引っ張られるとか）
   コンテキストファイルでは必要最小限の情報のみを書いておくので、別のファイルに引っ張られることはありません。
3. **詳細なレビューが可能**
   実装計画をコンテキストファイルとして保存しておくことで、より詳細なレビューが可能になります。
   これにより、複数のLLMによる相互レビューが可能となります。

### コンテキストエンジニアリングのやり方

1. コンテキストをクリア
2. Claudeにコンテキストファイルの生成を依頼
   Ex. 「ログイン機能を実装するコンテキストファイルを作成して」
3. 作成されたコンテキストファイルを、その他LLMや人がレビューを行う
4. 再度コンテキストをクリア後、LLMに実装を依頼
